 ...
 * 2、调用wait()后，当前线程会释放对象锁，当前线程进入对象的锁等待池中，直到别的线程调用对象的notify或notifyAll,当前线程进入
 *    对象的锁池中，去竞争对象锁，如果没有竞争到锁，那么线程一直留在锁池中
 * 3、notify：随机唤醒对象锁等待池中一个线程；notifyAll:唤醒对象锁等待池中所有线程
 * 4、线程的6种状态
 */
